...
export namespace Configuration {
  ...
  export function fromEnvironment(
    ...
    if (envVariables.API_KEY_X_API_KEY) {
      config.apiKeyCredentials = {
        'X-API-Key': envVariables.API_KEY_X_API_KEY,
      };
    }

    if (envVariables.BEARER_AUTH_ACCESS_TOKEN) {
      config.bearerAuthCredentials = {
        accessToken: envVariables.BEARER_AUTH_ACCESS_TOKEN,
      };
    }
...
const configurationSchema: Schema<Partial<Configuration>> = object({
  ...
  apiKeyCredentials: [
    'apiKeyCredentials',
    optional(object({ 'X-API-Key': ['X-API-Key', string()] })),
  ],
  bearerAuthCredentials: [
    'bearerAuthCredentials',
